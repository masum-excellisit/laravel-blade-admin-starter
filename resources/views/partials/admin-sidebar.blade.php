...
$nav = [
    ...
    ['label' => 'Users', 'route' => 'admin.users.index', 'icon' => 'M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 10-4-4 4 4 0 004 4z', 'can' => 'users.view'],
    ...
    @foreach($nav as $item)
        @if($item['can'] === null || auth()->user()->can($item['can']))
            ...
            <a href="{{ route($item['route']) }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition {{ $active ? 'brand-gradient text-white shadow-lg shadow-primary/20' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
