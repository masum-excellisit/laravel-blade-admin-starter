    ...
    <x-card title="Details">
        <form method="POST" action="{{ route('admin.profile.update') }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="flex items-center gap-4">
                <span class="h-16 w-16 rounded-full brand-gradient text-white text-xl font-semibold flex items-center justify-center">{{ $user->initials() }}</span>
                <div class="flex-1"><x-form.input name="avatar" type="file" label="Avatar" accept="image/*" /></div>
            </div>
